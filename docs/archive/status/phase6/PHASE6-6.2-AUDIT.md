...
已经全部修复，仅剩1个问题：12.⚠️ EmitExprForStore(Expr *) — 文档中定义了但未实现
...
--EmitCXXNewExpr: malloc + 数组 cookie + in-place 构造
--EmitCXXDeleteExpr: 数组析构循环 + free
...
## 类型系统根源性重构（new/delete 全面规范化）

### 已消除风险

| 风险 | 消除方式 |
|------|---------|
| P0: Sema 不处理 new/delete | CodeGenModule::SemaPostProcessAST 在 CodeGen 前遍历 AST 设置 ExprTy |
| P1: PointerType 不去重 | ASTContext 新增 PointerTypeCache/LValueRefTypeCache/RValueRefTypeCache |
| P2: QualType 无 getAsString() | 基于 dump() 基础设施实现 getAsString() |
| P3: Cookie 偏移硬编码 | ArrayCookie::CookieSize 常量集中管理 |
| P4: CXXNewExpr::getType() 为 null | SemaPostProcessAST 设置 ExprTy = T* |

### 新增 API

- `QualType::getAsString()` — 类型名序列化
- `CXXDeleteExpr::getAllocatedType()` — 被删除元素类型
- `ArrayCookie::CookieSize` — 数组分配 cookie 大小常量
- `ASTContext` PointerType/ReferenceType 去重缓存
- `Sema::ActOnCXXNewExpr()` / `Sema::ActOnCXXDeleteExpr()` / `Sema::ProcessAST()`
- `CodeGenModule::SemaPostProcessAST()` — CodeGen 入口处的 Sema 后处理

### 修改文件

`Type.h/cpp`, `ASTContext.h/cpp`, `Expr.h/cpp`, `Sema.h/cpp`, `ParseExprCXX.cpp`, `CGCXX.h`, `CodeGenModule.h/cpp`, `CodeGenExpr.cpp`
...
6. ✅ new[] / delete[] 已实现
--AST: CXXNewExpr 新增 AllocatedType 字段
--Parse: parseCXXNewExpression 传入解析后的 Type
--EmitCXXNewExpr: 数组形式分配 sizeof(size_t) + sizeof(T)*n，cookie 存数组长度
--EmitCXXNewExpr: 有析构函数的类逐元素调用默认构造函数
--EmitCXXDeleteExpr: 数组形式读取 cookie → 逆序析构循环 → free 原始指针
--EmitCXXDeleteExpr: 单元素形式先析构再 free
--CGCXX: 新增 GetDestructor()/EmitDestructorCall() 辅助方法

7. ✅ 构造函数调用 — 已修复（Stage 6.4 ✅ + new in-place 构造 ✅）
--EmitCXXConstructExpr 真正调用 CreateCall(CtorFn, {this, args...})
--基类/成员初始化器已实现（CGCXX.cpp EmitBaseInitializer/EmitMemberInitializer）
--✅ new T(args) 在 malloc 地址上 in-place 构造
--遗留：copy elision（P2）

8. ✅ 异常处理 — 已完整实现 Itanium C++ ABI 异常机制（P2）
--✅ try/catch 有完整 invoke + landingpad + resume 框架（CodeGenStmt.cpp）
--✅ EmitCallExpr 在 try 块内生成 invoke 而非 call
--✅ throw: 调用 __cxa_allocate_exception + memcpy + __cxa_throw（含 RTTI typeinfo）
--✅ throw;: 调用 __cxa_rethrow
--✅ catch(T e): 使用 RTTI typeinfo 作为 landingpad clause（非 catch-all）
--✅ catch handler: 使用 __cxa_begin_catch / __cxa_end_catch 管理异常生命周期
--✅ EmitCXXConstructExpr / EmitCXXNewExpr 使用 EmitCallOrInvoke（try 块内走 invoke）
--✅ EmitDestructorCall 使用 EmitNounwindCall（析构函数不应抛异常）
...
4. ✅ GotoStmt 已实现
--LabelMap (DenseMap<const LabelDecl*, BasicBlock*>) 提供 Label → BasicBlock 映射
--getOrCreateLabelBB 支持前向引用（goto 先于 label 定义）
--EmitGotoStmt 生成 br 指令跳转到目标 BB
--EmitLabelStmt 注册 BB 到 LabelMap 并正确发射

5. ⚠️ EmitIfStmt 缺少条件变量支持（Parse 层未实现）
--Clang 支持 if (int x = expr) 语法
新增 parseCondition() 方法 (src/Parse/ParseStmt.cpp):

检测条件中是否为声明形式（type name = expr）
支持 int/bool/float/double/char/auto 等内置类型和用户定义类型
解析为 VarDecl + DeclRefExpr 条件表达式
修改 parseIfStatement/parseSwitchStatement/parseWhileStatement:

使用 parseCondition() 替代 parseExpression()
将 CondVar 传递给 Sema 层
Sema 层 (include/blocktype/Sema/Sema.h + src/Sema/Sema.cpp):

ActOnWhileStmt 增加 VarDecl *CondVar 参数
ActOnSwitchStmt 增加 VarDecl *CondVar 参数
传入 AST 构造器
CodeGen 层 — 无需修改（已有 EmitCondVarDecl 支持）

6. ✅ EmitCXXTryStmt 已实现 invoke + landingpad 模式
--使用 invoke 指令替代 try 块内的 call（CodeGenExpr.cpp 中 isInTryBlock() 判断）
--landingpad + catch clause 分派异常
--resume 指令传播未捕获异常
--catch 块从 landingpad 结果提取异常对象（catch(...) 和 catch(T e) 两种形式）

## -----------------------------------------------------------

## 通用问题

1. ✅ 缺少 ExprValueKind 感知（已修复 — EmitExpr 入口增加 isGLValue() 分派，对 glvalue 表达式统一走 EmitLValue+load）
--AST 定义了 VK_PRValue/VK_LValue/VK_XValue
--CodeGen 已根据 value kind 区分 lvalue/rvalue 语义

2. ✅ 枚举常量类型硬编码为 i32（已修复 — CodeGenConstant.cpp 使用 ConvertType(ECD->getType()) 获取底层类型）
--EmitDeclRefExpr 中已在 C 节问题 9 修复
--CodeGenConstant.cpp 常量折叠路径已同步修复

3. ✅ 结构体返回值已处理（已修复 — EmitFunctionBody sret 参数映射与返回处理，EmitCallExpr sret 调用传入缓冲区）
--大型结构体按值返回通过 sret 参数传递
--EmitFunctionBody 跳过 sret 隐式首参数，返回 void
--EmitCallExpr 为 sret 函数创建缓冲区作为首参数
...
6. ✅ dynamic_cast 运行时检查已实现
--RTTI 基础设施完整（EmitTypeInfo 生成 _ZTS/_ZTI 全局变量）
--EmitDynamicCast 实现 null check → vtable RTTI → __dynamic_cast
--引用类型 dynamic_cast<T&> 失败时调用 __cxa_bad_cast()
--非多态类型 fallback 为 static bitcast
7. ✅ new[] / delete[] 已实现（根源性重构）
--AST: CXXNewExpr 新增 AllocatedType 字段，Parse 层传入解析类型
--AST: CXXDeleteExpr 新增 AllocatedType 字段，Parse 层推导元素类型
--Sema: 新增 ActOnCXXNewExpr/ActOnCXXDeleteExpr/ProcessAST 方法
--CodeGenModule: SemaPostProcessAST 在 CodeGen 前遍历 AST 设置 ExprTy
--CXXNewExpr: Sema 设置 ExprTy=T*，EmitCXXNewExpr 使用 getType() 得到指针类型
--CXXDeleteExpr: Sema 设置 ExprTy=void，EmitCXXDeleteExpr 使用 getAllocatedType() 得到元素类型
--EmitCXXNewExpr: new T[n] 分配 CookieSize + sizeof(T)*n，cookie 存数组长度
--EmitCXXNewExpr: 有析构函数的类元素调用默认构造函数循环
--EmitCXXNewExpr: 单元素 new T(args) 在 malloc 内存上调用构造函数（in-place 构造）
--EmitCXXDeleteExpr: delete[] 读取 cookie 获取长度，逆序调用析构函数循环，free 原始指针
--EmitCXXDeleteExpr: 单元素 delete 先调用析构再 free
--CGCXX 新增 GetDestructor()/EmitDestructorCall() 辅助方法
--ArrayCookie 常量集中管理，消除硬编码耦合
8. ✅ 构造函数调用遗留（已修复 — copy elision 已实现）
--✅ EmitCXXConstructExpr 真正调用 CreateCall(CtorFn, {this, args...})
--✅ new T(args) 在 malloc 地址上 in-place 构造
--✅ copy elision: EmitCXXConstructExpr 增加 DestPtr 参数，EmitVarDecl 检测 prvalue 直接在 alloca 上构造
9. ✅ 异常处理已完整实现（__cxa_allocate_exception / __cxa_throw / __cxa_begin_catch / __cxa_end_catch / RTTI dispatch）
10. Alloca 插入点效率优化（使用 AllocaInsertPt 成员）
11. 条件表达式 bool 转换精度
12. 缺少 Cleanups 栈
13. ✅ 指针差值已实现（pointer - pointer → ptrdiff_t，Sema+CodeGen 两层修复）
14. ✅ GotoStmt 已实现（LabelMap + getOrCreateLabelBB + 前向引用）
15. ✅ EmitCXXTryStmt 已实现（invoke + landingpad + resume）
