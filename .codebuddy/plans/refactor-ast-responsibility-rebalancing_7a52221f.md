---
name: refactor-ast-responsibility-rebalancing
overview: 将 AST 节点的创建、类型设置职责从 CodeGen/Parse 集中回 Sema 模块，消除 CodeGen 对 AST 的修改操作，建立清晰的编译管线分层。
todos:
  - id: remove-codegen-sema
    content: 从 CodeGenModule 删除 SemaPostProcessAST/SemaVisitStmt/SemaVisitExpr 及其调用
    status: completed
  - id: activate-sema-driver
    content: 在 driver.cpp 中创建 Sema 实例并调用 ProcessAST
    status: completed
    dependencies:
      - remove-codegen-sema
  - id: build-test
